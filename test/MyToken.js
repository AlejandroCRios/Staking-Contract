...
describe("Lock", function () {
  ...
  describe("Fee collection", function () {
    it("Should collect fees on P2P", async function () {
      const { uniswapRouter, myToken, vaultWallet, walletA, walletB } =
        await loadFixture(launchAndFundWalletA);
      await myToken
        .connect(walletA)
        .transfer(walletB.address, ethers.utils.parseEther("100"));
      expect(ethers.utils.parseEther("95.0")).to.equal(
        await myToken.balanceOf(walletB.address)
      );
      expect(ethers.utils.parseEther("5.0")).to.equal(
    ...
    it("Should collect fees on Buy", async function () {
      ...
      await myToken
        .connect(walletA)
        .approve(uniswapRouter.address, ethers.utils.parseEther("100.0"));
      await uniswapRouter
        .connect(walletA)
      ...
      expect(ethers.utils.parseEther("10.0")).to.equal(
        await myToken.balanceOf(vaultWallet.address)
      );
    });
    ...
    it("Should collect fees on Sell", async function () {
      ...
      expect(ethers.utils.parseEther("15.0")).to.equal(
        await myToken.balanceOf(vaultWallet.address)
      );
    });
  });
